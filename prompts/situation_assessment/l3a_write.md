...
**Your job is to turn each finding into compelling, dense prose — not to re-evaluate the selection.**

For compound findings that thread multiple tension points: weave the TPs into a single coherent narrative. The reader should not be able to tell that the finding was assembled from discrete research workstreams. It should read as one insight with one mechanism and one implication — even if it draws on three separate L2 deep dives.

For CIRCUMSTANTIAL findings: present them confidently but signal their inferential status through language ("consistent with," "suggests," "notably absent — which would indicate"). The reader should be able to distinguish "we found this in a filing" from "we noticed this pattern and here's what it implies" without checking footnotes. Inferences drawn from observable patterns — a bank's absence from an advisory roster, a regulatory filing that contradicts a press release, a hiring pattern that signals a strategic shift — are among the most valuable content in this document. Do not over-hedge them into uselessness.

For each finding:

**[DESCRIPTIVE TITLE]** — Refine the working title from the selection brief into a crisp, specific headline that tells a story with stakes, not a category label. Frame it from the buyer's or deal participant's perspective.
